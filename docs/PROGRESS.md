...
- **Help & guide page**: an in-app documentation page (`/help`, Volt, linked from the account dropdown, open to all authed users) covering roles, the core quote→job→pick→invoice→payment flow, inventory/picking, assets, scheduling/board, field work, agreements, getting paid, settings, and the API. Static content, no DB.
- **Canadian payments tweak**: added `PaymentMethod::Eft` ("EFT / direct deposit") alongside Interac e-Transfer — reflecting how Canadian trades actually get paid (e-Transfer auto-deposits to the bank; you record the deposit + reference). Roadmap payments section reframed Canada-first (Interac/EFT/Rotessa over US-centric card gateways).
- **Document appearance settings** (the pragmatic "editable print/PDF templates"): new `company_settings.document_terms` (a **Terms & conditions** block) + `show_tax_number` toggle (migration `2026_06_16_000032`). Both wired into all four document views — `print/invoice`, `print/quote`, `pdf/invoice`, `pdf/quote` — so the terms block prints/attaches and the GST/HST number can be shown or hidden (Canadian tax-invoice compliance). Editable in Company Settings. A full WYSIWYG template editor is deliberately out of scope (unsafe/large); branding (logo/accent/footer) already flowed in.
- **API writes**: the JSON API now has write endpoints — `POST /api/v1/customers`, `PATCH /api/v1/customers/{id}`, `POST /api/v1/jobs`. They validate input (422 on bad data) and **enforce the same role gates as the app** via the token's user (`Gate::allows('manage-customers'|'manage-jobs')`), so a read-only/viewer token gets 403. Tenant-scoped like the reads. *Webhooks (outbound events) remain a follow-up.*
- **In-app notifications**: Laravel database notifications (`notifications` table, migration `2026_06_16_000031`; `User` already `Notifiable`). `JobAssigned` notification fires from `jobs/show` `assign()` to the newly-assigned tech (not when self-assigning). `notifications` Volt page lists them, mark-read / mark-all-read; nav shows an "Alerts" link with an unread-count badge. First notification type; more (overdue invoice, low stock) can reuse the pattern. Notifications are scoped to the user (whose row is tenant-scoped).
- **Global search**: `search` Volt page (route `/search`, `#[Url] q`, live debounced input, "Search" nav link) that queries customers, jobs, invoices, and inventory items (min 2 chars, ~8 each) and links straight to each record's show page. Tenant-scoped via `BelongsToTenant`. No migration.
- **Back-orders feed the reorder view**: the reorder page (`inventory/reorder`) now has a "Back-ordered from picks" section — short-picked pick-list lines (`short_quantity > 0`) on jobs that are still open (not Done/Cancelled), grouped by item with total quantity back-ordered and the job numbers needing them. Closes the loop from short-pick → reorder. No migration.
- **Server-side PDF attachments**: emailed invoices/quotes attach a real PDF. DomPDF-friendly views `pdf/invoice.blade.php` + `pdf/quote.blade.php` (inline-styled HTML — DomPDF ignores Tailwind). `InvoiceMail`/`QuoteMail` `attachments()` render the PDF via `Barryvdh\DomPDF\Facade\Pdf`, **guarded by `class_exists`** so the app runs (and tests pass) without the package; the attachment activates once `composer require barryvdh/laravel-dompdf` is run on the host. Tests render the PDF *views* (dependency-free). *This is the "true PDF" upgrade over the browser-print views, which remain for on-screen/print.*
- **Customer sign-off / signature capture**: on the job, a canvas (Alpine, `wire:ignore`) captures a customer signature; `saveSignature($dataUrl)` (`work-jobs`) validates the PNG data URL, base64-decodes it, stores it at `signatures/{tenant}/…png`, and stamps `signature_path`/`signed_by_name`/`signed_at` on `service_jobs` (migration `2026_06_16_000030`). Shows the signed image + name + time; office (`manage-jobs`) can clear it. The signature is drawn client-side; the decode/store/stamp action is what's tested.
- **Payment receipt emails**: recording a payment optionally emails the customer a branded receipt (`PaymentReceiptMail` + `emails/receipt.blade.php`), routed through `TenantMailer`. New editable template type `payment_receipt` (tokens: `payment_amount`, `payment_method`, `payment_date`, `invoice_balance`, …) in Settings → Email templates. A "Email a receipt to …" checkbox on the record-payment form (shown only when the customer has an email, default on). No migration.
