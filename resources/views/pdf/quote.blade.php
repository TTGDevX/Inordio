...
<body>
    <table>
        <tr>
            ...
            <td class="right">
                <div style="font-size:16px; font-weight:bold;">QUOTE</div>
                <div class="muted">{{ $quote->number }}</div>
                @if ($quote->valid_until)<div class="muted">Valid until {{ $quote->valid_until->format('M j, Y') }}</div>@endif
            </td>
        </tr>
    </table>

    <p style="margin-top:16px;"><span class="muted">Prepared for</span><br><strong>{{ $quote->customer->name }}</strong></p>
    ...
    <table class="totals" style="margin-top:12px; width:40%; margin-left:60%;">
        <tr class="grand"><td>Subtotal (pre-tax)</td><td class="right">${{ number_format($quote->subtotal(), 2) }}</td></tr>
    </table>
